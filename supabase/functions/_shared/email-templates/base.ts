// [EMAIL:TEMPLATE] Template de base Foxeo — layout HTML responsive
...
export function baseTemplate(content: BaseTemplateContent): string {
  const cta = content.ctaUrl
    ? `<a href="${content.ctaUrl}" style="display:inline-block;margin-top:24px;padding:12px 24px;background:#059669;color:#ffffff;border-radius:6px;text-decoration:none;font-family:'Poppins',sans-serif;font-weight:600;">${content.ctaText ?? 'Voir sur Foxeo'}</a>`
    : ''
...
<body style="margin:0;padding:20px;background:#f4f4f5;font-family:'Inter',Arial,sans-serif;">
  <div style="max-width:600px;margin:0 auto;background:#ffffff;border-radius:8px;padding:32px;">
    ...
    <p style="font-size:12px;color:#a1a1aa;margin:16px 0 0;">
      Vous recevez cet email car vous êtes inscrit sur Foxeo.
