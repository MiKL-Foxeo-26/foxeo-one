...
| T-022a | Reste-a-faire de T-022 : rendre les messages MenuFacile visibles sur l'ACCUEIL. Le panneau « Messages non lus » ne lit que la table `messages` du chat MPP ; les contacts MenuFacile viennent d'un guichet HTTP externe. Deux systemes distincts — les fusionner change le sens du panneau. Question produit posee a MiKL : compteur unique « tout ce qui attend une reponse » OU deux lignes distinctes ? | Bug | Convergence T-022 2026-09-09 | Should | Interne | M | S | En attente MiKL |  — | 2026-09-09 | — | 2026-09-09 |
...
| Trancher (FORGE, T-020) : le kit `auth-supabase-next` a bien un `safeNext`, mais DUPLIQUE a 2 endroits et PLUS FAIBLE que le garde-fou ecrit aujourd'hui — il laisse passer `/\site-externe` (antislash normalise en slash par certains navigateurs) et `/javascript:...`. Proposition : remplacer les 2 copies par un `sanitizeReturnPath` unique et strict + ses 7 tests, et journaliser au CHANGELOG du kit. Cout ~15 min. Ce n'est PAS un nouveau kit a extraire — c'est un renforcement de securite d'un kit existant | MiKL | T-020 | 2026-09-09 |
| Trancher (T-022a) : les messages MenuFacile sur l'accueil — compteur UNIQUE « tout ce qui attend une reponse, d'ou que ca vienne » ou DEUX lignes distinctes (un client MPP et un utilisateur d'un produit tiers ne s'adressent pas a MiKL de la meme facon) ? 10 min dans les deux cas | MiKL | T-022a | 2026-09-09 |
